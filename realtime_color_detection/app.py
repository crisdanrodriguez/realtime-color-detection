"""Core application for real-time HSV color detection."""
...
class HSVRange:
    ...
    def lower_array(self) -> np.ndarray:
        """Return the lower bound as an unsigned byte NumPy array."""
        return np.array(self.lower, dtype=np.uint8)

    def upper_array(self) -> np.ndarray:
        """Return the upper bound as an unsigned byte NumPy array."""
...
def ensure_runtime_dependencies() -> None:
    ...
    if missing_packages:
        ...
        raise RuntimeError(
            ...
            f"{package_list}. Install them with `pip install .` or "
...
def open_camera(
    ...
    """Open the webcam using the requested backend preference."""
...
def create_trackbars(bounds: HSVRange) -> None:
    """Create the OpenCV trackbar controls for HSV tuning."""
...
def set_trackbars(bounds: HSVRange) -> None:
    """Synchronize UI controls with a normalized HSV range."""
...
def read_trackbars() -> HSVRange:
    """Read and normalize the current HSV range from the trackbars."""
    bounds = HSVRange(
        lower=(
...
def preprocess_frame(frame: np.ndarray, blur_kernel: int) -> np.ndarray:
    """Apply optional blur before HSV conversion."""
    if blur_kernel <= 1:
        return frame
...
def build_mask(hsv_frame: np.ndarray, bounds: HSVRange, morph_kernel: int) -> np.ndarray:
    """Build the binary mask for the selected HSV range."""
...
def add_overlay(
    ...
    """Render runtime overlay information on top of the live camera frame."""
